...
export default function Navbar() {
  ...
  // 檢查 localStorage，預設為 light
  const [theme, setTheme] = useState(localStorage.getItem('theme') || 'light');
  const isLoggedIn = !!tokenUtils.getRefreshToken();
  ...
  return (
    <div className="navbar bg-base-100 shadow-sm px-4 sm:px-8 border-b border-base-200 sticky top-0 z-50">
      ...
      <div className="flex-none gap-2">
        ...
        {isLoggedIn && (
          <button onClick={handleLogout} className="btn btn-ghost hover:bg-error/20 hover:text-error transition-colors">
            登出
          </button>
        )}
      </div>
